...
# Ubuntu-specific: Define configuration directories
confDir="${HOME}/.config"
cacheDir="${HOME}/.cache"
...
# GitHub repository cloning function (mostly unchanged)
if [ -d "$2" ]; then
    Theme_Dir="$2"
else
    Git_Repo=${2%/}
    if echo "$Git_Repo" | grep -q "/tree/"; then
        ...
        # Ubuntu-specific: Ensure jq is installed
        if ! command -v jq &> /dev/null; then
            print_prompt -y "[WARN] " "jq not found. Installing..."
            sudo apt-get update
    ...
    if [ -d "$Theme_Dir" ]; then
        ...
        # Ubuntu-specific: Ensure git is installed
        if ! command -v git &> /dev/null; then
            print_prompt -y "[WARN] " "git not found. Installing..."
            sudo apt-get update
...
# Rest of the script remains largely the same as the Arch version
...
# The rest of the script remains the same as the Arch version

# check_tars function (unchanged)
check_tars() {
    local trVal
    local inVal="${1}"
...
# Rest of the script remains the same as the Arch version
